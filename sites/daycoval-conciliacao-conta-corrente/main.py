                    ...
                    if _id not in ids_vistos:
                        ...
                        if ficha:
                            ...
                            if processos and isinstance(processos, list) and len(processos) > 0:
                                ...
                                match _materia_cod:
                                    case 1:
                                        ...
                                        jec_civ = item['dados_processo']['materia_sigla'] = 'CIV'
                                    case 2:
                                        item['dados_processo']['materia_descricao'] = 'CÍVEL - JUIZADO ESPECIAL'
                                        jec_civ = item['dados_processo']['materia_sigla'] = 'JEC'

                                    case _:
                                        pdb.set_trace() #debug verificar materia_descricao
                            ...
                            if resultado_situacao == 2:
                                ...
                                valor_exito = next(
                                        (item['TRC_SETENCA_FAVORAVEL'] for item in _dados_tabela if 'TRC_SETENCA_FAVORAVEL' in item),
                                        None
                                    )

                                texto_aviso = "SETENCA FAVORAVEL"
                            
                            elif resultado_situacao == 3:

                                valor_exito = next(
                                        (item['TRC_SETENCA_DESFAVORAVEL'] for item in _dados_tabela if 'TRC_SETENCA_DESFAVORAVEL' in item),
                                        None
                                    )
                                
                                texto_aviso = "SETENCA DESFAVORAVEL"
                            
                            elif resultado_situacao == 7 or resultado_situacao == 8:

                                valor_exito = next(
                                        (item['TRC_SEM_RESOLUCAO_MERITO'] for item in _dados_tabela if 'TRC_SEM_RESOLUCAO_MERITO' in item),
                                        None
                                    )
                                
                                texto_aviso = "SEM RESOLUCAO DE MERITO_OU_COM RESOLUCAO DE MERITO"

                            elif resultado_situacao == 6 or resultado_situacao == 4 or resultado_situacao == 5:

                                valor_exito = 0
                                texto_aviso = f"DESISTENCIA_OU_ACORDO_PRE_OU_POS_SENTENCA {resultado_situacao}"
                                indefinido = True

                            else:

                                pdb.set_trace() #debug verificar_resultado_situacao, verificar se o resultado retornado é o correto, etc


                            valor_a_receber = valor_defesa + valor_exito

                            if indefinido == False:

                                if valor_recebido < valor_a_receber:

                                    item['conciliacao_errada'] = 'sim'
                                    item['valor_divergencia'] = valor_recebido - valor_a_receber
                                    item['a_fazer'] = f'Verificar lançamentos faltantes, pois valor recebido ({valor_recebido}) é menor ({valor_a_receber}) previsto na tabela de valores para {texto_aviso}'
                                    item['motivo_conciliacao_errada'] = f'Valor recebido ({valor_recebido}) menor que DEFESA E {texto_aviso} somadas: ({valor_a_receber})'
                                    print(f'      ✗ Conciliação errada: {item["motivo_conciliacao_errada"]} → divergencia={item["valor_divergencia"]}')
                                
                                elif valor_recebido == valor_a_receber:

                                    item['conciliacao_errada'] = 'nao'
                                    item['valor_divergencia'] = 0
                                    item['a_fazer'] = f'Valor recebido está correto, conforme soma de DEFESA E {texto_aviso} ({valor_a_receber}) previsto na tabela de valores para {texto_aviso}'
                                    item['motivo_conciliacao_errada'] = f'Valor recebido ({valor_recebido}) igual à soma de DEFESA E {texto_aviso} somadas: ({valor_a_receber})'
                                    ...
                                    item['conciliacao_errada'] = 'sim'
                                    item['valor_divergencia'] = valor_recebido - valor_a_receber
                                    item['a_fazer'] = f'Verificar lançamentos extras, pois valor recebido ({valor_recebido}) é maior que soma de DEFESA E {texto_aviso} ({valor_a_receber}) previsto na tabela de valores para {texto_aviso}'
                                    item['motivo_conciliacao_errada'] = f'Valor recebido ({valor_recebido}) maior que soma de DEFESA E {texto_aviso} somadas: ({valor_a_receber})'
                                    print(f'      ✗ Conciliação errada: {item["motivo_conciliacao_errada"]} → divergencia={item["valor_divergencia"]}')
                                ...
                                item['valor_divergencia'] = 0
                                item['a_fazer'] = f'Esse caso é de {texto_aviso}, mas valor recebido ({valor_recebido}) e a receber ({valor_a_receber}), então verificar detalhes do processo para entender o cenário'
                                item['motivo_conciliacao_errada'] = f'Caso é de {texto_aviso} valor recebido ({valor_recebido}) e a receber ({valor_a_receber}), então verificar detalhes do processo para entender o cenário'
