...
from sites.oleConsignado.ole_insercao.auto.executar_insercao import (
    preencher_dados_simulacao, preeencher_dados_cliente,
    preencher_dados_operacao, confirmar_aba_resumo
)
...
class OleConsigInsercao(Manager):
    ...
    def __init__(self, driver: Chrome=False):
        super().__init__()
        self.user = chrome_user('ole_consig')
    ...
    def iniciar_horario_comercial(cls, driver: Chrome):
        run = OleConsigInsercao(driver)
        try:
            run.executar_insercao()
    ...
    def executar_insercao(self):
        ...
        for cnt, info_contrato in enumerate(infos_contratos['contratos'], 1):
            print(f"[{cnt}]Fila Inserir Contratos")
            ...
            try:
                ...
                #self.chrome_driver.get(self.urls['aba_insercao'] + cpf_url)
                ...
                if info_contrato['tipo'] == 'REFINANCIAMENTO' or info_contrato['tipo'] == 'REFINANCIAMENTO SOMANDO MARGEM':
                    ...
                    try:
                        ...
                        print('Não encontrou matricula para refinanciamentos...')
                        atualizacoes['mensagem'] = "Reprovado a Conferir"
                        atualizacoes['erro'] = 'FAVOR CONFERIR. Não há a matricula e parcela disponível para refinanciamento'
                        atualizacoes['observacao'] = 'FAVOR CONFERIR. Não há a matricula e parcela disponível para refinanciamento'
                    ...
                    try:
                        ...
                        try: 
                            ...
                            atualizacoes['mensagem'] = "Reprovado a Conferir"
                            atualizacoes['erro'] = 'FAVOR CONFERIR. Parcela não disponível para refinanciamento'
                            atualizacoes['observacao'] = 'FAVOR CONFERIR. Parcela não disponível para refinanciamento'
                            self.data.atualizar_contrato(contrato['codigoContrato'], atualizacoes)
                            continue
                else:
                    try:
                ...
                atualizacoes: dict = preencher_dados_simulacao(self.chrome_driver, contrato, self.contrato_tipo_contrato)
                ...
                if atualizacoes == False or ('retorno' in atualizacoes and atualizacoes['retorno'] == False):
                    ...
                    #     #atualizacoes['textoMensagem'] = 'Informe no campo abaixo somente o token (código de 4 números) que foi enviado via mensagem de texto (SMS) em seu celular para que possamos consultar sua margem no INSS. Caso não tenha recebido peça abaixo o reenvio.'
                    ...
                    #     self.data.atualizar_contrato(contrato['codigoContrato'], atualizacoes)
                    #     print('Pulando insercao para aguardar o token')
                    #     continue
                    ...
                    contrato['codigoContrato'],  {'mensagem': 'Inserir contrato'})
                    
                    atualizacoes['mensagem'] = 'Reprovado a Conferir'

                    if('erro' in atualizacoes):
                        atualizacoes['erro'] = atualizacoes['erro']
                        atualizacoes['observacao'] = atualizacoes['erro']
                    else:                        
                        atualizacoes['erro'] = 'Parcela / portabilidade não disponível para refinanciamento'
                        atualizacoes['observacao'] = 'Parcela / portabilidade não disponível para refinanciamento'
                    self.data.atualizar_contrato(contrato['codigoContrato'], atualizacoes)
                else:

                    if('margem' in atualizacoes and atualizacoes['margem'] != ''):
                    ...
                    preeencher_dados_cliente(self.chrome_driver, contrato)

                    self.sessao.verificar_estado()

                    preencher_dados_operacao(self.chrome_driver, contrato)

                    self.sessao.verificar_estado()

                    if not confirmar_aba_resumo(self.chrome_driver, info_contrato['tipo']):
                        raise Exception("Não foi possível concluir inserção.")

                    print("Atualizando dados do contrato: ", atualizacoes)

                    self.data.atualizar_contrato(contrato['codigoContrato'], atualizacoes)
                    # self.data.gerar_log_insercao(
                    #     self.caminho_base+"/oleConsignado/ole_insercao",
                    #     codigo_con=contrato['codigoContrato'],
                    #     ade=None
                    # )
                    # self.data.api_registrar_log_robo(
                    #     log="Contrato inserido com sucesso.",
                    #     status=2
                    # )
                    continue
    ...
    def verifica_erro_tela(self,codigo_contrato):
        ...
        try:
            ...
            try:
                erro = self.act.obter_texto('//*[@id="divMensagemErro"]/ul/li',By.XPATH)
            except:
                pass
            pass
        ...
        if(erro):
            ...
            elif('Cliente inscrito na lista do Não Me Perturbe' in erro):
                mensagem = 'Reprovado a Conferir'
                textoMensagem = 'Como está na lista do Não me Pertube no momento não podemos realizar essa proposta.'  
...
if __name__ == '__main__':
    ...
    while True:
        OleConsigInsercao.executar_insercao()
        t.sleep(120)
