        ...
        if tipo_titulo == False:
            ...
            if integracao == True:
                ...
                if tarefas_processamento:
                        ...
                        for idx, tarefa in enumerate(tarefas_processamento, start=1):
                            ...
                            #pdb.set_trace()  # Debug para verificar dados_processo
            ...
            for idx, tarefa in enumerate(tarefas_ordenadas, start=1):
                ...
                if cliente_grupo_trabalho == 78:
                    ...
                    # forcar_integracoes = ['DAYCOVAL.337117']
                    ...
                    caminho_sucesso_daycoval = os.path.join(BASE_PATH, 'daycoval_lancamentos_sucesso.json')
                    id_tramitacao_sucesso = set()
                    try:
                        if os.path.exists(caminho_sucesso_daycoval):
                            with open(caminho_sucesso_daycoval, 'r', encoding='utf-8') as f_sucesso:
                        ...
                        print(f'⚠ Não foi possível carregar daycoval_lancamentos_sucesso.json: {sucesso_error}')
                    ...
                    filtro_tarefa_resultado_audiencia = {
                                                            "_and": [
                                                                {
                                                                    "evento": {
                                                                    "_eq": "$RA"
                    ...
                    tarefas_resultado_audiencia = api_buscar_processo_tarefa_filter(
                        filter_data=filtro_tarefa_resultado_audiencia,
                        limit=1,
                    )

                    qtd_resultado = len(tarefas_resultado_audiencia) if tarefas_resultado_audiencia else 0

                    if qtd_resultado == 1:

                        audiencia_cancelada = tarefas_resultado_audiencia[0].get('situacao_desc') == 'Não cumprimento'
                        if audiencia_cancelada == True:
                            print  ('Audiencia cancelada...')
                            continue
                    else:
                        pdb.set_trace() #tratar resultado tarefas_resultado_audiencia
                                        ...
                                        if numero_processo:
                                            try:
                                                ...
                                                if valor_audiencia_texto in lancamento_ja_realizado.text and texto_complemento in lancamento_ja_realizado.text:
                                                    ...
                                                    with open(caminho_sucesso_daycoval, 'w', encoding='utf-8') as f_sucesso:
                                                ...
                                                campo_historico.send_keys(texto_historico)
                                                campo_historico.send_keys(Keys.ENTER)
                                                ...
                                                time.sleep(2)   # Aguarda o fechamento do popup de upload
                    ...
                    print(f'  [{idx}/{len(tarefas_ordenadas)}] id_processo={id_processo} -> Grupo de trabalho não é 78, pulando busca por ATA.')
