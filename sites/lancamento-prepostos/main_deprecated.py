    ...
    if token:
        ...
        if tipo_titulo == False:
            ...
            for idx, tarefa in enumerate(tarefas_ordenadas, start=1):
                    ...
                    numero_integracao = tarefa.get('processo_detalhes', [{}])[0].get('numero_integracao')
                    id_processo = tarefa.get('id_processo')
                                ...
                                if sucesso_download and os.path.exists(arquivo_local):
                                    ...
                                    resp_audiencia = _requests.post(
                                        ...
                                        json={'base64': b64, 'mime_type': 'application/pdf', 'prompt' : """Você é um analisador de atas de audiência judiciais.
                                                ...
                                                - Se a audiência for híbrida, ou seja, permitir participação tanto presencial quanto virtual, considere como "audiencia_presencial": false.
                                                - Se mencionar que a presença foi feita de forma remota
                                                - Se mencionar aplicativo TEAMS
                                    ...
                                    if resp_audiencia.status_code == 200:
                                        try:
                                            texto = resp_audiencia.json()[0]['output'][1]['content'][0]['text']
                                        ...
                                        if url_ifood:
                                            ...
                                            if numero_processo:
                                                ...
                                                url = f'https://ifood.bclegal.io/ifoodv5/(S({SESSION_ID}))/default.aspx?RedirectV8=solicitacao/redireciona.aspx'
                                                driver_ifood.get(url)

                                                aguardar_loading_ifood(driver_ifood, 180)

                                                driver_ifood.switch_to.default_content()
                                                WebDriverWait(driver_ifood, 20).until(
                                                                                                    EC.frame_to_be_available_and_switch_to_it((By.ID, 'mainFrame'))
                                                                                                )

                                                                    

                                                campo_lote_codigo_interno = WebDriverWait(driver_ifood, 20).until(
                                                                                                        EC.presence_of_element_located(
                                                                                                            (By.ID, 'ctl00_ContentPlaceHolder1_ConsultaRapida_txtCampo1')
                                                                                                        )
                                                                                                    )
                                                campo_lote_codigo_interno.clear()
                                                campo_lote_codigo_interno.send_keys(lote)

                                                checkbox = WebDriverWait(driver_ifood, 20).until(
                                                    EC.presence_of_element_located(
                                                        (By.ID, 'ctl00_ContentPlaceHolder1_ConsultaRapida_chkCampo7')
                                                    )
                                                )

                                                if checkbox.is_selected():
                                                    driver_ifood.execute_script("arguments[0].click();", checkbox)

                                                campo_lote_codigo_interno.send_keys(Keys.ENTER)

                                                aguardar_loading_ifood(driver_ifood, 180)

                                                primeiro_id = WebDriverWait(driver_ifood, 20).until(
                                                    EC.presence_of_element_located(
                                                        (By.ID, 'ctl00_ContentPlaceHolder1_dgLista_ctl03_LinkButton1')
                                                    )
                                                ).text

                                                while primeiro_id != lote:
                                                    print(f'    ⚠ Lote não encontrado, tentando novamente: {primeiro_id} != {lote}')
                                                    time.sleep(5)
                                                    driver_ifood.refresh()
                                                    aguardar_loading_ifood(driver_ifood, 180)
                                                    primeiro_id = WebDriverWait(driver_ifood, 20).until(
                                                        EC.presence_of_element_located(
                                                            (By.ID, 'ctl00_ContentPlaceHolder1_dgLista_ctl03_LinkButton1')
                                                ...
                                                print(f'    ✓ Lote encontrado: {primeiro_id}')

                                                driver_ifood.execute_script("__doPostBack('ctl00$ContentPlaceHolder1$dgLista$ctl03$LinkButton1','')")

                                                try:
                                                    WebDriverWait(driver_ifood, 5).until(
                                                        EC.presence_of_element_located(
                                                            (By.ID, 'ctl00_cphPrincipal_txtNrProcesso')
                                                        )
                                                    )
                                                except:
                                                    print('    ✓ Reufazenro a url')
                                                    redefine_url_ifood(driver_ifood)

                                                lista_processos = WebDriverWait(driver_ifood, 20).until(
                                                    EC.presence_of_element_located((By.ID, 'ctl00_cphPrincipal_rowListaCasosRelacionados'))
                                                ).text 

                                                if numero_processo in lista_processos:
                                                    print(f'    ✓ Número do processo encontrado: {numero_processo}')
                                                    continue
                                                else:
                                                    print(f'    ⚠ Número do processo não encontrado: {numero_processo}')
                                                ...
                                                try:
                                                            ...
                                                            try:
                                                                ...
                                                                while retorno == False:
                                                                    ...
                                                                    try:
                                                                        ...
                                                                        campo_responsavel.click()
                                                                        driver_ifood.execute_script("window.scrollTo(0, 0);")
                                                                        ...
                                                                        aguardar_loading_ifood(driver_ifood, 90)
                                                                ...
                                                                try:
                                                                    ...
                                                                    try:
                                                                        try:
                                                                            tela_sucesso = WebDriverWait(driver_ifood, 10).until(
                                                                                EC.element_to_be_clickable(
                                                                            ...
                                                                            #     EC.frame_to_be_available_and_switch_to_it((By.ID, 'mainFrame'))
                                                                            # )
                                                                            botao_salvar = WebDriverWait(driver_ifood, 20).until(
                                                                                        EC.element_to_be_clickable(
                                                                                            (By.ID, 'ctl00_btnSalvar')
                                                                                        )
                                                                                    ).click()
                                                                        ...
                                                                        if 'Documento salvo com sucesso!' in tela_sucesso or 'Reembolso salvo com sucesso.' in tela_sucesso:
                                                    ...
                                                    pdb.set_trace()  # Debug: Captura de exceção durante o preenchimento do Ifood
                    ...
                    #DAYCOVAL #78 de grupo de trabalho
                        ...
                        # forcar_integracoes = ['DAYCOVAL.354846','DAYCOVAL.365246','DAYCOVAL.357584','DAYCOVAL.350897','DAYCOVAL.331712','DAYCOVAL.304853']
                        
                        # if numero_integracao not in forcar_integracoes:
                        #     print(f'  ⚠ Integração {numero_integracao} não está na lista de integrações forçadas para Daycoval. Pulando...')
                        #     continue
                                        ...
                                        if url_daycoval:
                                            ...
                                            if numero_processo:
                                                try:
                                                    ...
                                                    campo_empresa_interesse = WebDriverWait(driver_daycoval, 20).until(
                                                        EC.presence_of_element_located(
                                                            (By.ID, 'ctl00_ctl00_cphZoomMain_cphMain_fvwMain_empresaLookupInter_I')
                                                        )
                                                    )

                                                    campo_empresa_interesse.click()
                                                    campo_empresa_interesse.send_keys(Keys.CONTROL, 'a')
                                                    campo_empresa_interesse.send_keys(Keys.DELETE)

                                                    campo_empresa_interesse.send_keys('FINANCEIRA CONSIGNADO')
                                                    ...
                                                    pdb.set_trace() #debug após preenchimento dos campos do lançamento no Daycoval, antes de salvar
