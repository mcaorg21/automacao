...
class OmniConciliacaoContaCorrente(ProcessWrapper):
    def __init__(self):
    ...
    def _proxima_execucao_futura(self) -> bool:
        try:
            config_path = Path(project_path()) / "omni-conciliacao-conta-corrente/config.json"
    ...
    def _consumir_executar_agora(self) -> bool:
        """Retorna True e limpa o flag se executar_agora estiver ativo."""
        try:
            config_path = Path(project_path()) / "omni-conciliacao-conta-corrente/config.json"
            with open(config_path, encoding='utf-8') as f:
                config = json.load(f)
